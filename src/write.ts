...
*.local`
  ...
  rules: {}
        ...
        fs.writeFileSync(
            ".prettierrc",
            JSON.stringify({ semi: true, trailingComma: "all" }, null, 2)
        );
    }
    ...
    writeAppFile() {
        fs.writeFileSync("src/app" + getFileExtension(this.language),
            returnAppFile(this.language),
        );
    }

    writeIndexFile() {
        fs.writeFileSync(
            "src/index" + (getFileExtension(this.language)),
            returnIndexFile(this.language),
        );
    }

    writePingRoute() {
        let filepath = "src/routes/" + "ping" + getFileExtension(this.language);
        fs.writeFileSync(filepath, returnRouteData(this.language, "ping"))
    }

    writeSampleRoute() {
        let filepath = "src/routes/" + "sample" + getFileExtension(this.language);
        fs.writeFileSync(filepath, returnRouteData(this.language, "sample"))
    }

    writeRoutesFile() {
        let filepath = "src/routes/" + "index" + getFileExtension(this.language);
        this.writePingRoute();
...
`
    ...
    writeSampleController() {
        let filepath = "src/controllers/" + "sample" + getFileExtension(this.language);
        fs.writeFileSync(filepath, returnControllerData(this.language, "sample"));
    }

    writeControllersFile() {
        let filepath = "src/controllers/" + "index" + getFileExtension(this.language);
...
export default Controllers;\n`
    ...
    writeSchemaFile() {
        ...
        let filepath = "src/schemas/" + "index" + getFileExtension(this.language);
